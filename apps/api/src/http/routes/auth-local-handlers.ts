...
import { loginSchema, mfaBeginSchema, mfaCodeSchema, registerSchema } from './auth-local-schemas.js'
...
export async function handleMfaBegin(c: Context<AppEnv>) {
...
export async function handleMfaConfirm(c: Context<AppEnv>) {
...
export async function handleMfaVerify(c: Context<AppEnv>) {
  const parsed = mfaCodeSchema.safeParse(await c.req.json())
  if (!parsed.success)
    throw new ValidationError(parsed.error.issues[0]?.message ?? 'Invalid payload')
