...
>
> _Synthesized from: `SAD_Arquitectura_v2 (2).pdf` (SAD v1.0, 13 pages) and stack details from `MAME_SRS_v2.pdf` (SRS v2.0)_
...
### 4.1 Frontend: Next.js 15 + TypeScript + Tailwind CSS + shadcn/ui + React Bits
...
| **UI Components**          | **shadcn/ui** (Radix-based) — accessible, customizable primitives installed as source code in `components/ui/`. Includes Sidebar, Button, Input, Select, Card, Badge, etc. NOT a dependency — code is owned and modifiable.                            |
| **Visual Effects**         | **React Bits** — curated collection of animated components. Galaxy background for landing page. Installed via CLI as source code in `components/backgrounds/`. Same principle: code is yours to modify.                                                |
...
| **Why shadcn/ui**          | Provides production-ready accessible components without adding a runtime dependency. Code lives in `src/components/ui/` — fully customizable, follows project Tailwind theme. Radix primitives ensure WAI-ARIA compliance.                             |
| **Why React Bits**         | Curated animated backgrounds and effects that add visual polish. Galaxy background provides a distinctive landing page without complex custom animations. Source code is copied into project — no external runtime.                                    |
