...
>
> _Synthesized from: `SRS_Requerimientos_v2 (2).pdf` (SRS v1.0, 11 pages) and `MAME_SRS_v2.pdf` (SRS v2.0, 7 pages)_
...
| RF        | Description                                                                                                                                             | Priority | Sprint | Implementation Details                                                                                                                                                               |
| --------- | ------------------------------------------------------------------------------------------------------------------------------------------------------- | -------- | ------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| **RF-22** | **Anonymous comments** up to 1000 characters with same automatic filter pipeline as reports. 2-level nesting.                                           | HIGH     | S3     | **Comments are auto-published immediately** after validation + AI/rate-limit checks. They do **not** require prior human approval. Human moderation is triggered **by exception** (flags threshold, abuse patterns, or legal/critical AI signals). Comment voting supported. Max 2 levels of nesting (comment → reply, no deeper). |
...
| **BR-09** | Comments are public immediately after automated checks (validation + AI + anti-spam) | Human review is only required when comments are flagged or abuse thresholds are reached |
